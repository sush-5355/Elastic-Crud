...
def bulk_update_all(index_name, update_fields):
    ...
    search_result = es.search(
        index=index_name, body=search_body, scroll="2m", size=scroll_size
    )
    ...
    while len(search_result["hits"]["hits"]) > 0:
        ...
        for hit in search_result["hits"]["hits"]:
            ...
            bulk_data.append({"doc": update_fields})
...
def delete_index(index_name):
    ...
    return {
        "status": False,
        "msg": f"Index - {index_name} already deleted or does not exists",
    }
...
def delete(index_name, id):
    ...
    if index_exists:
        ...
        return {
            "status": True,
            "msg": f"Index = {index_name} bearing _id = {id} deleted successfully",
        }
    return {
        "status": False,
        "msg": f"Index - {index_name} already deleted or does not exists",
...
def get_all(index_name, size=None):
    result = es.search(
        index=index_name, body={"size": size, "query": {"match_all": {}}}
    )
    result = result["hits"]["hits"]
    count = len(result)
    final_list = []
...
def create(index_name, mapping: dict):
    ...
    try:
        ...
        response_update = update_by_id(
            index_name=index_name, id=response["_id"], doc=response_get_by_id
        )
...
def get_all_scroll(index_name, batch_size):
    ...
    search_body = {"query": {"match_all": {}}}
    search_result = es.search(
        index=index_name, body=search_body, scroll=scroll, size=batch_size
    )
